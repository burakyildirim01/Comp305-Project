...
def find_max_dp(curr, k):
    if dp[curr.key - 1][k] is None:
        ...
        if curr.right is None:
            if k > 0:
                dp[curr.key - 1][k] = max(curr.value, find_max_dp(curr.left, k) + curr.value)
                return max(curr.value, find_max_dp(curr.left, k) + curr.value)
            else:
                dp[curr.key - 1][k] = curr.value + find_max_dp(curr.left, 0)
                return curr.value + find_max_dp(curr.left, 0)
        ...
        if k > 0:
            ...
            if k > 1:
                dp[curr.key - 1][k] = max(find_max_dp(curr.left, k-1) + curr.value, find_max_dp(curr.right, k-1) + curr.value, curr.value + maximum,
                   curr.value)
                return max(find_max_dp(curr.left, k-1) + curr.value, find_max_dp(curr.right, k-1) + curr.value, curr.value + maximum,
                   curr.value)
            else:
                dp[curr.key - 1][k] = max(find_max_dp(curr.left, k-1) + curr.value, find_max_dp(curr.right, k-1) + curr.value, curr.value + maximum)
                return max(find_max_dp(curr.left, k-1) + curr.value, find_max_dp(curr.right, k-1) + curr.value, curr.value + maximum)
        else:
            dp[curr.key - 1][k] = find_max_dp(curr.left, 0) + curr.value + find_max_dp(curr.right, 0)
            return find_max_dp(curr.left, 0) + curr.value + find_max_dp(curr.right, 0)
    else:
